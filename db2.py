...
class GUI(ttk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.Device["vedbod"] = { "nbr" : 2, "status" : False, "name": "stople", "topic" : "vedbod/status/switch:0", "Elementid" : "",
            "picture-on":"vedbod-on.png", "picture-off" : "vedbod-off.png", "picture-disabled" : "vedbod-dis.png" }
        self.Device["garage"] = { "nbr" : 3, "status" : False, "name": "stople", "topic" : "garage/status/switch:0", "Elementid" : "",
            "picture-on":"garage-on.png", "picture-off" : "garage-off.png", "picture-disabled" : "garage-dis.png" }
        self.Device["stuga"]  = { "nbr" : 4, "status" : False, "name": "stople", "topic" : "stuga/status/switch:0",  "Elementid" : "",
        ...
        if message.topic == "house/env" :
            ...
            if "sensor_id" in js :
                if js['sensor_id'] == "outdoorTemp" :
                    ...
                    #self.thisTemp = float(js["properties"]["Temp"])
                    if OutdoorTemp > 0 and OutdoorTemp <= 5:
                        self.templabel.config(foreground="orange")
                    if OutdoorTemp < 5 :
                        self.templabel.config(foreground="green")
                    self.thistemp.set(str(OutdoorTemp) + degree_sign)
        ...
        if message.topic == "stolpe/status/switch:0" :
            try:
                if js.get('output') == True :
                    self.stolpeStatus = True
                    self.stolpe.configure(image = self.stolpeON)
                    logging.info("Stolpe on")
                else:
                    self.stolpeStatus = False
                    self.stolpe.configure(image = self.stolpeOFF)
                    logging.info("Stolpe off")
